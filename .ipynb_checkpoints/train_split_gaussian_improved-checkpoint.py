...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # 及时删除split_data引用以节省内存
        if 'split_data' in render_pkg:
            del render_pkg['split_data']
        ...
        if phase == 1:
            ...
            loss, loss_dict = compute_comprehensive_frequency_loss(
                rendered=image,
                gt=gt_image,
                wave_vectors=gaussians._wave if phase >= 2 else None,
        ...
        if phase == 3 and "split_viewspace_points" in render_pkg and render_pkg["split_viewspace_points"] is not None:
            ...
            if split_viewspace_points.grad is not None:
                ...
                # 设置原始viewspace_point_tensor的梯度
                viewspace_point_tensor.grad = aggregated_grad

        iter_end.record()

        with torch.no_grad():
            # Wave管理（阶段2和3）
            if phase >= 2 and gaussians._wave is not None:
                # 检查并修复NaN
                if gaussians._wave.isnan().any():
                    print(f"[Warning] NaN in wave at iteration {iteration}, resetting")
                    gaussians._wave.data[gaussians._wave.isnan()] = 0.0
                
                # 梯度裁剪
                if gaussians._wave.grad is not None:
                    torch.nn.utils.clip_grad_norm_([gaussians._wave], max_norm=1.0)
            ...
            # 标准梯度裁剪
            torch.nn.utils.clip_grad_norm_([gaussians._xyz, gaussians._scaling, 
                                           gaussians._rotation, gaussians._opacity], max_norm=1.0)
            ...
            if iteration % 10 == 0:
                ...
                if phase >= 2:
                    wave_norms = torch.norm(gaussians._wave, dim=1)
                    postfix_dict["Wave"] = f"{wave_norms.mean():.3f}"
                    postfix_dict["Active"] = f"{(wave_norms > 0.01).sum().item()}"
            ...
            # 内存监控
            # ==========================
            if log_memory_usage(iteration, current_phase_name):
                # 如果内存压力大，可以考虑降低分裂数量
                if phase == 3:
                    gaussians._max_splits = max(gaussians._max_splits - 1, 1)
                    print(f"[Memory] Reducing max_splits to {gaussians._max_splits}")
            ...
            if iteration < opt.densify_until_iter:
                ...
                # 密集化间隔（阶段3降低频率）
                densify_interval = opt.densification_interval
                if phase == 3:
                    densify_interval = int(densify_interval * 1.5)
                    # Phase 3中更保守的密度调整以减少内存使用
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    
                    if iteration > opt.densify_from_iter and iteration % densify_interval == 0:
                        gaussians.densify_and_prune_frequency_aware(
                            opt.densify_grad_threshold * 1.5,  # 提高阈值
                            0.005 * 0.8,  # 更宽松的剪枝
                            scene.cameras_extent,
                            size_threshold, iteration, opt.iterations
                        )
                        # 密度调整后清理
                        torch.cuda.empty_cache()
                else:
