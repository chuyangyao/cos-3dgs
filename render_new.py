...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, model_type="split_gaussian", render_dir=None):
    ...
    # 创建迭代次数子目录 - 修复：GT和renders都使用相同的迭代目录结构
    iter_dir = f"iter_{iteration}" if iteration > 0 else ""
    
    base_render_path = os.path.join(model_path, name, render_dir)
    
    # 修复：GT图像也保存在带迭代次数的子目录中，避免覆盖
    if iter_dir:
        render_path = os.path.join(base_render_path, "renders", iter_dir)
        gts_path = os.path.join(base_render_path, "gt", iter_dir)
    else:
        render_path = os.path.join(base_render_path, "renders")
    ...
    for idx, view in enumerate(tqdm(views, desc="渲染进度")):
        ...
        if model_type == "split_gaussian":
            ...
            # 分裂拉普拉斯方法（融合方案）使用render_new函数
            rendering = render_new(view, gaussians, pipeline, background)["render"]
        else:
            # 默认使用render函数
            rendering = render(view, gaussians, pipeline, background)["render"]
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool, model_type="split_gaussian", render_dir=None):
    ...
    with torch.no_grad():
        ...
        if not skip_train:
             render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, model_type, render_dir)

        if not skip_test:
             render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, model_type, render_dir)
